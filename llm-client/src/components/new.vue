<template>
    ...
    <div class="home">
      <div class="sidebar">
        ...
        <li v-for="(theme, index) in themes" :key="index">
          <a :href="'#'+theme.id" class="truncate-text"  @click.prevent="get_conversation(theme.id)">{{ theme.summary }}</a>
        </li>
      </ul>
      </div>
          ...
          <div class="container">
            <div class="left">
              <div v-for="(message, index) in wenxin_messages" class="main-message" :key="index"
                ...
                <div class="message-sender"
                  ...
                  <img v-else-if="message.sender === 'assistant'" src="@/assets/文心一言.png" alt="Friend Icon">
            ...
            <div class="mid">
              <div v-for="(message, index) in tongyi_messages" class="main-message" :key="index"
                ...
                <div class="message-sender"
                     :class="{'user-message': message.sender === 'user', 'friend-message': message.sender === 'assistant'}">
                  <img v-if="message.sender === 'user'" src="@/assets/我的.png" alt="User Icon">
                  <img v-else-if="message.sender === 'assistant'" src="@/assets/通义千问.png" alt="Friend Icon">
                  <span class="message-sender-name"
                        :class="message.sender === 'user' ? 'user-color' : 'friend-color'">{{ message.sender }}:</span>
                </div>
                <div v-if="message.sender === 'user'" class="user-message">{{message.content }}</div>
            ...
            <div class="right">
              <div v-for="(message, index) in chatgpt_messages" class="main-message" :key="index"
                   :class="{'user-message': message.sender === 'user', 'friend-message': message.sender === 'system'}">
                <!-- 显示用户标识和图片 -->
                <div class="message-sender"
                     :class="{'user-message': message.sender === 'user', 'friend-message': message.sender === 'system'}">
                  <img v-if="message.sender === 'user'" src="@/assets/我的.png" alt="User Icon">
                  <img v-else-if="message.sender === 'system'" src="@/assets/chatgpt.png" alt="Friend Icon">
                  <span class="message-sender-name"
                        :class="message.sender === 'user' ? 'user-color' : 'friend-color'">{{ message.sender }}:</span>
                </div>
                <div v-if="message.sender === 'user'" class="user-message">{{ message.content }}</div>
    ...
    methods: {
      ...
      get_conversation(themeId){
        ...
        .then(data => {
          ...
          copy_chatgpt_messages=data.chatgpt_messages;
          copy_chatgpt_messages.forEach(element => {
            
            if(element.role=='system'){
              element.content=this.md.render(element.content);
  
            }
            this.chatgpt_messages.push(element);
          });
  
          copy_tongyi_messages=data.tongyi_messages;
          copy_tongyi_messages.forEach(element => {
            
            if(element.role=='assistant'){
              element.content=this.md.render(element.content);
  
            }
            this.tongyi_messages.push(element);
          });
  
          copy_wenxin_messages=data.wenxin_messages;
          copy_wenxin_messages.forEach(element => {
            
            if(element.role=='assistant'){
              element.content=this.md.render(element.content);
  
            }
            this.wenxin_messages.push(element);
          });
      ...
      CreateNewAndSave() {
        if (this.current_id == null) {
            ...
            // 如果当前ID不为null，更新已有对话
            fetch(`/api/conversations/update_conversation/${this.current_id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
      ...
      async handleSearch() {
        ...
        try {
          ...
          this.wenxin_eventSource = new EventSource('http://127.0.0.1:5000/wenxin?query=' + keyword,{ withCredentials: true });
          // 设置消息事件监听器
          this.wenxin_eventSource.onmessage = (event) => {
            try {
              ...
              if (dataObject.message != 'done') {
                ...
                this.wenxin_messages[this.wenxin_messages.length-1]=wenxin_friendMessage;
              }
              this.scrollToBottom();
            } catch (e) {
          ...
          // 创建一个新的 EventSource 实例
          this.tongyi_eventSource = new EventSource('http://127.0.0.1:5000/tongyi?query=' + keyword,{ withCredentials: true });
          // 设置消息事件监听器
          this.tongyi_eventSource.onmessage = (event) => {
            try {
              ...
              if (dataObject.message != 'done') {
                ...
                this.tongyi_messages[this.tongyi_messages.length-1]=tongyi_friendMessage;
              }
              this.scrollToBottom();
            } catch (e) {
              console.error('Error parsing JSON:', e);
          ...
          // 创建一个新的 EventSource 实例
          this.chatgpt_eventSource = new EventSource('http://127.0.0.1:5000/chatgpt?query=' + keyword,{ withCredentials: true });
          // 设置消息事件监听器
          this.chatgpt_eventSource.onmessage = (event) => {
            try {
              ...
              if (dataObject.message != 'done') {
                ...
                this.chatgpt_messages[this.chatgpt_messages.length - 1] = chatgpt_friendMessage;
              }
              this.scrollToBottom();
            } catch (e) {
              console.error('Error parsing JSON:', e);
  ...
  .sidebar-header h2 {
    margin: 0;
    font-size: 20px;
    font-family: "楷体", "KaiTi", serif; /* 楷体字体 */
  }
